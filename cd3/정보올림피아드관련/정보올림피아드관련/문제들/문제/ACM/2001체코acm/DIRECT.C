...
static int fx[MAXQ][MAXQ],fy[MAXQ][MAXQ] ; /* link to the next field */
...
ifstream in("direct.in");
ofstream out("direct.out");

int solve_subtaskx(int x1,int y1,int x2,int y2)
{ int z,edge ;
  int dx,z1,z2,dz,ry,t,y,dy,ry2 ;

  //    if (x1==3 && x2==6) printf("solve_subtaskx called with x1=%d y1=%d x2=%d y2=%d\n",x1,y1,x2,y2) ;
  ...
  for (t=x1;t<x2;t++) {
    ...
    //if (x1==3 && x2==6 && y1=) {   printf("Testing x=%d y=%3g ry=%d h[t][ry]=%d h[t+1][ry]=%d z=%g\n",
    //     t,(double)y/(2*dx),ry,h[t-1][ry-1],h[t][ry-1],(double)z/(2*dx)) ;
    // 	printf("dx=%d y+dx-1=%d\n",dx,y+dx-1) ; 
    //}
...
{ int z,edge ;
  ...
  //if (x1==3 && x2==3) printf("solve_subtasky called with x1=%d y1=%d x2=%d y2=%d\n",x1,y1,x2,y2) ;
  ...
  for (t=y1;t<y2;t++) {
    ...
    //    if (x1==3 && x2==3)  printf("Testing y=%d x=%3g rx=%d h[rx][t]=%d h[rx][t]=%d z=%g\n",
    //         t,(double)x/(2*dy),rx,h[rx-1][t-1],h[rx-1][t],(double)z/(2*dy)) ;
...
{ int l ;
  ...
  //            if (l) printf("Can see from (%d,%d) to (%d,%d)\n",x1,y1,x2,y2) ;
  //	        else printf("Cannot see from (%d,%d) to (%d,%d)\n",x1,y1,x2,y2) ;
  return l ;
}
